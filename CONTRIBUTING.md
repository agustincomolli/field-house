...
3. Si tocás `windows/engine/FieldHouseEngine.cs`, `windows/engine/Build-Engine.ps1`, `Install.ps1` o `Uninstall.ps1` (Windows):
   ...
   - Si agregás algo a `FieldHouseEngine.cs` que tenga equivalente en `bin/change_wallpaper.sh` (o viceversa), mantené los nombres de configuración alineados: `Ciudad`↔`CIUDAD`, `ModoHorarios`↔`MODO_HORARIOS`, etc. — es lo que permite que la documentación de "Cómo funciona" sea común a ambas plataformas.
