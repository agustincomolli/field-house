...
3. Si tocás `windows/Change-Wallpaper.ps1`, `Install.ps1` o `Uninstall.ps1` (Windows):
   - El CI del repo corre `PSScriptAnalyzer`, un parseo de sintaxis (AST) y smoke tests con `-DryRun` en un runner `windows-latest`, en el job `powershell-checks`. Si tenés PowerShell disponible localmente, adelantalo con `Invoke-ScriptAnalyzer -Path windows\ -Recurse -Severity Warning,Error -ExcludeRule PSAvoidUsingWriteHost` antes de abrir el PR.
   - Probá la lógica de franjas/clima con `-DryRun` (funciona igual que `--dry-run` en Linux: no toca el fondo real ni escribe logs). Si el cambio toca `SystemParametersInfo` o las Tareas Programadas, probalo también con una instalación real — eso sigue sin cobertura automatizada (el runner de CI no tiene una sesión de escritorio real donde verificar el cambio visual).
