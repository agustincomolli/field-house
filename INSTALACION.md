...
La ubicación no se configura acá: se detecta automáticamente por IP en cada ejecución (ver la sección "Ubicación" más abajo). Acordate: los valores que faltan en un `config.conf` viejo se cubren con los valores por defecto del script, así que agregar variables nuevas no rompe instalaciones existentes.
...
- **Automático** (`MODO_HORARIOS="auto"`): las franjas se calculan según la salida y puesta **real** del sol en tu ubicación, detectada automáticamente (ver sección siguiente).
...
En cada ejecución (con caché diaria, para no molestar a la API), se consulta la API oficial **Sunrise 3.0** de MET Norway (`api.met.no/weatherapi/sunrise/3.0/sun`) con las coordenadas de la ubicación detectada, y se extraen la salida (`sunrise`) y la puesta (`sunset`) del sol del día:
...
El resultado se guarda en `~/.local/state/field-house/horarios-sol.cache` y se reutiliza durante el día (se vuelve a consultar al cambiar la fecha). Si la consulta falla (sin internet, sin ubicación disponible), el script avisa en el log y usa los horarios fijos de `config.conf`.

> ⚠️ **Zona horaria.** El cálculo asume que el reloj de tu equipo está en la misma zona horaria que tu ubicación real (el caso normal). La API devuelve la hora ya ajustada al huso horario del lugar consultado, así que si viajás y tu reloj no se actualiza junto con la ubicación detectada, las franjas podrían no coincidir con el fondo esperado; en ese caso usá el modo fijo o ajustá la zona horaria del sistema.

### Ubicación

La ubicación geográfica **no se configura**: se detecta automáticamente por IP en cada ejecución, usando la API gratuita `ip-api.com`. No hay ningún paso de instalación que la pregunte, ni ningún campo en `config.conf` para editarla.

- **Se refresca en cada `--reboot`** (inicio de sesión), con el mismo mecanismo de reintentos que el clima (ver más abajo): si la red todavía no está lista, reintenta cada `ESPERA_REINTENTO_CLIMA` segundos hasta `REINTENTOS_CLIMA_INICIAL` veces.
- **El resultado se cachea en `~/.local/state/field-house/ubicacion.cache`, sin expiración por tiempo.** Esto es distinto a los cachés de clima y horario solar (que expiran por TTL o por día): acá, si `ip-api.com` no responde, se sigue usando la última ubicación conocida sin importar su antigüedad, en vez de quedarse sin ubicación. La lógica es que una notebook normalmente no cambia de ciudad de un día para el otro, así que una ubicación de hace unos días sigue siendo mucho más útil que ninguna. El caché solo se actualiza cuando la consulta realmente tiene éxito.
- **Solo queda sin ubicación disponible** si nunca hubo una geolocalización exitosa (instalación nueva, sin caché previo) y la consulta actual también falla — típicamente, una instalación recién hecha sin conexión a internet. En ese caso, esa ejecución usa horarios fijos y no hay clima disponible, igual que ante cualquier otro fallo de red; se resuelve solo apenas haya conectividad.

### Clima

En cada ejecución se consulta la API oficial **Locationforecast 2.0 (compact)** de MET Norway (`api.met.no/weatherapi/locationforecast/2.0/compact`) con las coordenadas de la ubicación detectada, y se extrae el `symbol_code` del pronóstico más inmediato (`next_1_hours`, con `next_6_hours` como respaldo). La consulta usa un timeout corto (`--connect-timeout 2 --max-time 6`): no espera más de 6 segundos en total, y no se cuelga en DNS. El clima nublado y la lluvia se manejan como condiciones aparte y usan imágenes distintas:

- **Nublado** (`cloudy`, `fair`, `partlycloudy`), se reemplaza el fondo "base" de la franja por la versión nublada:
...
- **Lluvia** (`rain`, `sleet`, `snow`, `thunder`, `fog`), se reemplaza por la versión lluviosa:
  - Amanecer o mediodía lluvioso → `lluvia-dia.jpg`
  - Atardecer lluvioso → `lluvia-atardecer.jpg`
  - Noche lluviosa → `lluvia-noche.jpg`

Si falla la consulta (sin internet, timeout superado, sin ubicación disponible), se usa el fondo base de la franja sin intentar lluvia ni nublado, y queda registrado en el log.

**Caché de clima:** como la consulta es una llamada de red, el resultado se guarda en `~/.local/state/field-house/clima.cache` y se reutiliza durante `TTL_CACHE_CLIMA` segundos (10 minutos por defecto). Esto evita consultas redundantes cuando el timer horario y el servicio de login disparan casi en el mismo momento — algo que, a su vez, respeta la política de uso justo de la API de met.no.

**Reintento al iniciar sesión:** al arrancar la PC la red (en particular el WiFi) puede tardar en estar lista, a veces más que la espera inicial de `ESPERA_INICIAL_SEGUNDOS`. Por eso, en la ejecución de login (flag `--reboot`), si la geolocalización o la consulta de clima fallan se aplica ya el fondo base de la franja y se reintenta cada `ESPERA_REINTENTO_CLIMA` segundos (60s por defecto) hasta `REINTENTOS_CLIMA_INICIAL` veces (3 por defecto) — el mismo mecanismo de espera cubre ambas consultas, porque las dos dependen de que la red esté lista. Si un reintento tiene éxito, el fondo pasa al de clima con la transición normal; si se agotan, queda el fondo base hasta el próximo disparo horario. En las ejecuciones normales (timer horario) no hay reintento: un fallo se deja para la próxima hora.
...
| Smoke test modo auto | `--dry-run` con `MODO_HORARIOS=auto` contra las APIs de met.no/ip-api.com reales (conectividad del runner) |
| Smoke test config completa | `--dry-run` con todas las variables nuevas explícitas en `config.conf` |
| Smoke test config inválida | Confirma que un `MODO_HORARIOS` inválido hace fallar el script (exit != 0) |
...
| Smoke test modo auto | `FieldHouseEngine.exe --dry-run` con `ModoHorarios: "auto"` contra las APIs de met.no/ip-api.com reales |
| Smoke test config inválida | Confirma que un `ModoHorarios` inválido hace fallar el ejecutable |
...
La versión Windows (`windows/engine/FieldHouseEngine.cs`, `Install.ps1`, `Uninstall.ps1`) replica la misma lógica de negocio que la versión Linux — franjas horarias, modo `auto` según el sol, ubicación autodetectada, clima con caché y reintentos, validación de configuración, rotación de log — con los mismos nombres conceptuales (las claves de `config.json` son PascalCase de las mismas variables de `config.conf`: `ModoHorarios` ↔ `MODO_HORARIOS`, `TtlCacheClima` ↔ `TTL_CACHE_CLIMA`, etc.). Todo lo de las secciones "Ubicación", "Franjas horarias", "Clima" y "Validaciones y comportamiento robusto" de más arriba aplica igual en Windows; acá se documenta solo lo que **cambia** por ser una plataforma distinta.
...
| Consultas HTTP | `curl` | `HttpWebRequest` (met.no para clima y horarios del sol, ip-api.com para ubicación) |
...
Probá `curl "https://api.met.no/weatherapi/locationforecast/2.0/compact?lat=TU_LAT&lon=TU_LON" -H "User-Agent: test"` directamente en la terminal (con tus coordenadas reales, que podés ver en `~/.local/state/field-house/ubicacion.cache`) para ver qué `symbol_code` devuelve, y ajustá la lista de palabras clave en `bin/change_wallpaper.sh` (nublado: `cloudy|fair|partlycloudy`; lluvia: `rain|sleet|snow|thunder|fog`) si tu clima devuelve un símbolo distinto. Recordá que el resultado se reutiliza 10 minutos por el caché (`TTL_CACHE_CLIMA`); si querés verificar un cambio, podés borrar `~/.local/state/field-house/clima.cache` y correr el script de nuevo.

**El log dice `AVISO: no se pudo obtener la salida/puesta del sol`:**
Pasa con `MODO_HORARIOS="auto"` cuando la consulta a la API Sunrise 3.0 de met.no falla o no hay ubicación disponible. No es grave: el script usa los horarios fijos de `config.conf` para esa corrida y lo intenta de nuevo en la próxima. Verificá internet o probá `curl "https://api.met.no/weatherapi/sunrise/3.0/sun?lat=TU_LAT&lon=TU_LON&date=$(date +%F)" -H "User-Agent: test"` con tus coordenadas reales.
...
**El log dice `ERROR: MODO_HORARIOS inválido` (o `HORA_INICIO_... inválida`):**
El script valida la configuración antes de aplicar nada. Editá el valor indicado en `~/.config/field-house/config.conf` con el formato que pide el mensaje (`MODO_HORARIOS` debe ser `fijo` o `auto`; horas en `HH:MM` de 24 hs) y corré el script de nuevo.
...
**La ubicación detectada automáticamente no es la correcta:**
La ubicación se detecta por geolocalización de IP (`ip-api.com`), que puede desviarse varios kilómetros según tu proveedor de internet — es normal que no sea exacta, especialmente si tu ISP asigna la IP desde otra ciudad. No hay forma de corregirla a mano (no hay ningún campo de ubicación en `config.conf`: se autodetecta siempre). Si te afecta el modo automático de horarios o el clima de forma notoria, usá `MODO_HORARIOS="fijo"` mientras tanto.
...
**`Install.ps1` falla en el paso "1/3 - Instalando archivos" con un error de `csc.exe`:**
...
**El clima no se detecta bien:**
Probá `Invoke-WebRequest "https://api.met.no/weatherapi/locationforecast/2.0/compact?lat=TU_LAT&lon=TU_LON" -Headers @{"User-Agent"="test"}` en PowerShell (con tus coordenadas reales, visibles en `%LOCALAPPDATA%\FieldHouse\state\ubicacion.cache.json`) y mirá `.Content` para ver el `symbol_code` que devuelve. Los mismos criterios de palabras clave que en Linux aplican (nublado: `cloudy|fair|partlycloudy`; lluvia: `rain|sleet|snow|thunder|fog`), ajustables en `windows/engine/FieldHouseEngine.cs` (método `ClimaCoincide`, requiere recompilar con `Build-Engine.ps1` tras el cambio). El caché de clima vive en `%LOCALAPPDATA%\FieldHouse\state\clima.cache.json`; borralo si necesitás forzar una consulta nueva.

**El log dice `AVISO: no se pudo obtener la salida/puesta del sol`:**
Mismo caso que en Linux: pasa con `"ModoHorarios": "auto"` cuando la consulta a la API Sunrise 3.0 de met.no falla o no hay ubicación disponible. El motor usa los horarios fijos de `config.json` para esa corrida.

**El log dice `ERROR: ModoHorarios inválido` (o `HoraInicio... inválida`):**
Corré `& "$env:LOCALAPPDATA\FieldHouse\bin\FieldHouseEngine.exe" --config` para reconfigurar paso a paso con validación en el momento, o editá el campo correspondiente a mano en `%APPDATA%\FieldHouse\config.json` con el formato que pide el mensaje (`ModoHorarios` debe ser `"fijo"` o `"auto"`; horas en `HH:MM` de 24 hs) y volvé a correr el motor.
...
**La ubicación detectada automáticamente no es la correcta:**
Igual que en Linux: es geolocalización por IP (`ip-api.com`), puede desviarse varios kilómetros según tu proveedor de internet. No hay forma de corregirla a mano (no hay ningún campo de ubicación en `config.json`: se autodetecta siempre, en cada ejecución). Si te afecta de forma notoria, usá `"ModoHorarios": "fijo"` mientras tanto.
